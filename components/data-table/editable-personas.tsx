...
export function EditablePersonas({ initialValue, row, column }: EditablePersonasProps) {
  ...
  const renderPersonas = () => {
    if (selectedPersonas.length === 0) {
      return <span className="text-table text-muted-foreground">Select personas</span>
    }

    return (
      <div className="flex items-center gap-1">
        <Badge variant="outline" className="text-table h-6 px-2">
          {selectedPersonas[0]}
        </Badge>

        {selectedPersonas.length > 1 && (
          <Badge variant="secondary" className="text-table h-6 px-2 ml-1">
            +{selectedPersonas.length - 1}
          </Badge>
        )}
      </div>
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-[250px] p-0">
        <Command className="text-table">
          <CommandInput placeholder="Search personas..." className="h-9 text-table" />
          <CommandList>
            {selectedPersonas.length > 0 && (
              <>
                <CommandGroup heading="Selected" className="text-table">
                  <div className="flex flex-wrap gap-1 p-2">
                    {selectedPersonas.map((persona) => (
                      <Badge key={persona} variant="outline" className="text-table px-2 py-1 flex items-center gap-1">
                        {persona}
                        <X
            ...
            <CommandEmpty className="text-table">No persona found.</CommandEmpty>
            <CommandGroup heading="All Personas" className="text-table">
              {allPersonas.map((persona) => (
                ...
                >
                  ...
                  />
                  <span className="text-table">{persona}</span>
                </CommandItem>
              ))}
            </CommandGroup>
          </CommandList>
